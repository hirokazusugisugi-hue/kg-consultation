...
function handleStatusChange(rowIndex, oldStatus, newStatus) {
  ...
  if (newStatus === STATUS.CONFIRMED) {
    // ── 重複防止: ポータル経由で既にメール送信済みかチェック ──
    var venueConfirmedFlag = null;
    try {
      venueConfirmedFlag = PropertiesService.getScriptProperties().getProperty('VENUE_CONFIRMED_ROW_' + rowIndex);
    } catch (propErr) {
      console.error('PropertiesService読み取りエラー:', propErr);
    }

    if (venueConfirmedFlag) {
      var elapsed = Date.now() - parseInt(venueConfirmedFlag);
      // 60秒以内のフラグがあればポータル経由でメール送信済み → スキップ
      if (elapsed < 60000) {
        console.log('ポータル経由で確定処理済み（二重送信防止）: 行' + rowIndex + ' (' + elapsed + 'ms前)');
        try {
          PropertiesService.getScriptProperties().deleteProperty('VENUE_CONFIRMED_ROW_' + rowIndex);
        } catch (e) { /* ignore */ }
        return;
      }
      // 60秒超過のフラグは古いので削除して通常処理を続行
      try {
        PropertiesService.getScriptProperties().deleteProperty('VENUE_CONFIRMED_ROW_' + rowIndex);
      } catch (e) { /* ignore */ }
    }
    ...
    if (!data.confirmedDate) {
      ...
      if (fullDateTime) {
        ...
        try {
          ...
          console.log('UI alert不可（トリガー/WebApp経由）: 確定日時未入力のためステータスを戻します');
        }
    ...
    if (!isOnline) {
      ...
      if (!location || location === '') {
        try {
          ...
          console.log('UI alert不可（トリガー/WebApp経由）: 場所未設定のためステータスを戻します');
        }
        sheet.getRange(rowIndex, COLUMNS.STATUS + 1).setValue(oldStatus || STATUS.PENDING);
        return;
      }
    ...
    if (isOnline && !data.zoomUrl) {
      ...
      if (zoomUrl) {
        try {
          ...
          console.log('UI alert不可（トリガー/WebApp経由）: Zoom自動作成完了 URL=' + zoomUrl);
        }
      } else {
        try {
          SpreadsheetApp.getUi().alert(
          ...
          console.log('UI alert不可（トリガー/WebApp経由）: Zoom自動作成失敗');
        }
      }
    }
    ...
    // 担当者・管理者への通知（buildStaffNotificationEmail_を使用）
...
function sendStaffReminderWithMembers_(rowData, daysBeforeLabel) {
  ...
  // P列の担当者に通知
  if (rowData.staff) {
    sendStaffNotifications(rowData.staff, emailSubject, emailBody);
    // 送信済みメールアドレスを記録（重複防止）
...
function sendScheduledStaffNotification() {
  ...
  // P列の担当者
  if (data.staff) {
    sendStaffNotifications(data.staff, emailResult.subject, emailResult.body);
