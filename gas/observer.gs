...
/**
 * 予約済みの相談予定を取得（拡張版：メンバー・オブザーバー情報付き）
 */
function getUpcomingConsultations() {
  ...
  for (var i = 1; i < bookingData.length; i++) {
    ...
    if ((status === '確定' || status === 'NDA同意済') && confirmedDate) {
      ...
      if (cDate >= weekAgo) {
          ...
          if (sDate) {
            try {
              ...
              if (sDateFormatted === dateRaw) {
                members = scheduleData[j][SCHEDULE_COLUMNS.MEMBERS] || '';
                break;
              }
        ...
        // NDA提出済みオブザーバーを取得
        var observers = ndaObservers[dateRaw] || [];
  ...
  // 日程設定シートから予約済みスロットも取得（予約管理に未登録のもの）
  for (var j = 1; j < scheduleData.length; j++) {
      ...
      if (sDate) {
        try {
          ...
          if (schedDate >= weekAgo2) {
            ...
            if (!exists) {
              var observers2 = ndaObservers[dateRaw2] || [];
              results.push({
                date: dateStr,
                dateRaw: dateRaw2,
                company: '（予約管理シート参照）',
                staff: sStaff || '（未定）',
                members: sMembers ? String(sMembers) : '',
